...
namespace JengrePetroleum.Module.DatabaseUpdate;
...
public class Updater : ModuleUpdater
{
    ...
    public override void UpdateDatabaseAfterUpdateSchema()
    {
        ...
        Employee userAdmin = ObjectSpace.FirstOrDefault<Employee>(u => u.UserName == "JengreAdmin");
        if (userAdmin == null)
        {
            userAdmin = ObjectSpace.CreateObject<Employee>();
            userAdmin.UserName = "JengreAdmin";
            // Set a password if the standard authentication type is used
    ...
    private EmployeeRole CreateDefaultRole()
    {
        ...
        if (defaultRole == null)
        {
            ...
            defaultRole.AddObjectPermissionFromLambda<EmployeeBase>(SecurityOperations.Read, cm => cm.Oid == (Guid)CurrentUserIdOperator.CurrentUserId(), SecurityPermissionState.Allow);
            defaultRole.AddNavigationPermission(@"Application/NavigationItems/Items/Default/Items/MyDetails", SecurityPermissionState.Allow);
            defaultRole.AddMemberPermissionFromLambda<EmployeeBase>(SecurityOperations.Write, "ChangePasswordOnFirstLogon", cm => cm.Oid == (Guid)CurrentUserIdOperator.CurrentUserId(), SecurityPermissionState.Allow);
            defaultRole.AddMemberPermissionFromLambda<EmployeeBase>(SecurityOperations.Write, "StoredPassword", cm => cm.Oid == (Guid)CurrentUserIdOperator.CurrentUserId(), SecurityPermissionState.Allow);
